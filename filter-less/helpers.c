...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
   ...
   {
    image[i][j].rgbtRed=(image[i][j].rgbtRed+image[i-1][j].rgbtRed+image[i+1][j].rgbtRed+image[i][j-1].rgbtRed+image[i-1][j-1].rgbtRed+image[i+1][j+1].rgbtRed+image[i][j].rgbtRed+image[i][j].rgbtRed+image[i][j].rgbtRed)/9;
   }
}
